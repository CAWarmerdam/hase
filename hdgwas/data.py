...
class ParData(Data):
    ...
    def get(self, gen_order=None, phen_order=None, cov_order=None):
        ...
        if isinstance(self.a_inv, type(None)):
            # This should work for higher dims of a_test with interaction stuff
            return self.a_test[indices_a_test], self.b_cov[
                indices_b_cov], self.C[phen_order], self.a_cov[indices_a_cov]
        else:
            return self.a_inv[indices_a_test], self.b_cov[
                indices_b_cov], self.C[phen_order], self.a_cov[indices_a_cov]
...
class MetaPhenotype(object):
    ...
    def __init__(self, phen, protocol=None, include=None, exclude=None):
        ...
        if protocol is None:
            for i, reader in enumerate(phen):
                phen_names = []
                if i == 0:
                    for j in reader.folder.files:
                        if j != 'info_dic.npy':
                            phen_names = phen_names + list(reader.folder.data_info[j])
    ...
    def get(self):
        if self.processed == self.n_phenotypes:
            ...
            self.processed = finish
            # Create empty numpy array with as a shape, the individual ids along the first dimension,
            # and the phenotype ids along the second dimension.
            phenotype = np.zeros(
                (np.sum([len(self.pool[i].folder._data.id) for i in self.pool]), len(range(start, finish))))
        # Now loop through the keys that resemble the indices of the readers
        for i, j in enumerate(self.keys):
            # Fill the phenotype array with values from every reader.
            if i == 0:
...
class MetaParData(object):
    ...
    def __init__(self, pd, study_names, protocol=None):
        ...
        def _check(map, keys):
            ...
            if np.sum(r) == values.shape[0]:
                raise ValueError('There is no common names between studies')
            # Loop through the keys, getting only those phenotypes that
            # have complete data from every study name.
            for i, k in enumerate(keys):
                result[k] = values[~r, i]
            # Return a dict with for every study, the order of phenotypes,
            # excluding those phenotypes that lack complete data
            return result
    ...
    def check_maf(self, SNPs_index):
        maf = np.zeros((SNPs_index[0].shape[0], len(self.pd_folders)))

        for i, j in enumerate(self.pd_folders):
            maf[:, i] = np.array(self.pd_folders[j].folder._data.metadata['MAF'])[SNPs_index[i].astype(np.int64)]
    ...
    def get(self, variant_indices=None, B4=False, regression_model=None, random_effect_intercept=False):
        """
        Method function yet to be determined.
        It could be that this function returns the partial derivatives
        that correspond to the given variant indices.
        :param variant_indices: The variants to return something for
        :param B4:
        :param regression_model:
        :param random_effect_intercept:
        :return:
        """
        # Throw an error if there are partial derivatives folders.
        if self.pd_folders is None:
            raise ValueError('Data not defined!')
        # Get the study names
        study_names = self.pd_folders.keys()
        # If the SNP indices are present
        if variant_indices is not None:
            # The number of pd folders and the number of
            # arrays with variant indices should match
            if len(self.pd_folders) != len(variant_indices):
                raise ValueError('There are not equal number of PD and SNPs indexes {}!={}'.format(len(self.pd_folders),
                                                                                                   len(variant_indices)))
            # Get the a_test, b_cov, C and a_cov that correspond
            ...
            if random_effect_intercept and len(self.pd_folders) > 1:  # TODO (high)
                a_test_effect = a_test[:, 0:1] # Get the first column, intercepts?
                b_cov_effect = b_cov[0:1, :] # Gets the first row, intercept?
                a_cov_effect = a_cov[0:1, :] # Gets the first row, intercept?
        else:
            raise ValueError('Indexes are not defined!')

        if B4:
            b4 = self.pd_folders[study_names[0]].folder._data.b4[variant_indices[0], :]
            b4 = b4[:, self.phen_order[study_names[0]]]

        # self.check_maf(SNPs_index)

        # Loop through the studies' partial derivatives starting from the 2nd one
        for i in range(1, len(self.pd_folders)):
            # Get ... for every partial derivatives folder
            a, b, c, a_c = self.pd_folders[study_names[i]].get(gen_order=variant_indices[i],
                                                     phen_order=self.phen_order[study_names[i]],
                                                     cov_order=self.cov_order[study_names[i]])
            # Report some stuff to the stdout
            self.check_pd([a_test, b_cov, C, a_cov], [a, b, c, a_c])
            # Merge the variable part of matrix A
            a_test = a_test + a
            # Merge the constant part of matrix B
            b_cov = b_cov + b
            # Merge C
            C = C + c
            # Merge the constant part of matrix A
            a_cov = a_cov + a_c
            if random_effect_intercept and i < (len(self.pd_folders) - 1):
            ...
            if B4:
                b4_tmp = self.pd_folders[study_names[i]].folder._data.b4[variant_indices[i], :]
                b4 = b4 + b4_tmp[:, self.phen_order[study_names[i]]]

        if random_effect_intercept and len(self.pd_folders) > 1:
            a_cov_I = np.zeros((len(self.pd_folders) - 1, len(self.pd_folders) - 1))
    ...
    def maf_pard(self, SNPs_index=None):
        ...
        for j, i in enumerate(self.pd_folders):
            n = len(self.pd_folders[i].folder._data.metadata['id'])
            samples += n
            maf = maf + n * np.array(self.pd_folders[i].folder._data.metadata['MAF'])[SNPs_index[j]]
        maf = maf / np.float(samples)
        return maf

    def get_n_id(self):
        return np.sum([len(self.pd_folders[i].folder._data.metadata['id']) for i in self.pd_folders])

    def get_phenotype_names(self):
        n = {}
        for i in self.pd_folders:
            n[i] = self.pd_folders[i].folder._data.metadata['phenotype']
        ...
        if self.folder_cache_flag and self.folder_cache.get(file) is not None and len(
            ...
            if self._id is None:
                ...
                if (self._id != self._data.id).any():
                    # TODO (middle) check for not overlapped id (AttributeError: 'bool' object has no attribute 'any')
